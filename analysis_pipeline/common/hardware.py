"""Hardware/runtime utilities for Hopper-class training runs.

This module is the single source of truth for:
- conservative CPU thread environment defaults
- Hopper-class CUDA arch handling
...
class HardwareInfo:
    ...
    def is_hopper(self) -> bool:
        ...
        return self.compute_capability == (9, 0) or "h100" in name or "hopper" in name
...
def conservative_thread_count() -> int:
    ...
    High-core-count Hopper-class systems expose many cores; we intentionally under-commit BLAS/OMP
...
def configure_cuda_arch_list(*, for_extension_build: bool) -> str:
    ...
    On Hopper-class SM90 systems, we never hard-code Blackwell 12.0.
