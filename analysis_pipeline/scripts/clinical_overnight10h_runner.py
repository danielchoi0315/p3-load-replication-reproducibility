...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--clinical_bids_root", type=Path, default=Path("/project_data/clinical_bids"))
    ap.add_argument("--clinical_severity_csv", type=Path, default=Path("/project_data/clinical_bids/clinical_severity.csv"))
    ap.add_argument("--features_root_healthy", type=Path, default=Path("/project_cache/features/confirmatory_reference"))
...
def main() -> int:
    ...
    if args.out_root is None:
        ...
        out_root = Path("/project_runs") / f"{ts}_CLINICAL_OVERNIGHT10H"
    else:
    ...
    if features_root_clinical is None:
        features_root_clinical = Path(f"/project_cache/features_CLINICAL_OVERNIGHT10H_{run_ts}")
