...
VENV_DIR="/project_root/analysis_pipeline/.venv"
...
# git-annex 8.x can become unstable at -J>1 on large OpenNeuro trees; use single-job staging by default.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "Unknown arg: $1"
      echo "Usage: bash scripts/run_full_pipeline.sh [--resume] [--out_root /project_runs/<ts>] [--seeds 0,1,2,3,4]"
      exit 1
      ;;
  esac
done
...
if [[ -n "$OUT_ROOT_ARG" ]]; then
  ...
  OUT_ROOT="/project_runs/${RUN_TS}"
fi

if [[ ! -d /project_data ]]; then
  echo "[ERROR] Required fast local disk path missing: /project_data"
  exit 1
fi

if ! mkdir -p "$LOG_ROOT" "$DATA_ROOT" "$FEATURES_ROOT" /project_runs; then
  echo "[ERROR] Unable to create required directories under /project_data (permission or mount issue)"
  exit 1
fi
...
on_exit() {
  ...
  echo "=============================================="
  if [[ $rc -eq 0 ]]; then
    echo "run_full_pipeline.sh completed successfully"
  else
    echo "run_full_pipeline.sh failed with exit code $rc"
  fi
...
run_stage "preflight" bash -lc '
  ...
  free_bytes=$(df -PB1 /project_data | awk "NR==2{print \$4}")
  free_gb=$(python - <<PY
b = int("${free_bytes}")
print(f"{b/1024**3:.2f}")
PY
...
PY
  ...
  echo "Free space on /project_data: ${free_gb} GB (${free_tb} TB)"
  ...
  shopt -s nocasematch
if [[ ! "${gpu_name}" =~ h100|hopper ]] && [[ ! "${gpu_cc}" =~ ^9\. ]]; then
  ...
  echo "[WARN] TORCH_CUDA_ARCH_LIST was set to ${TORCH_CUDA_ARCH_LIST}; unsetting for Hopper-class runtime"
...
name_l = name.lower()
if cc[0] != 9 and all(k not in name_l for k in ("h100", "hopper")):
    raise SystemExit("[ERROR] torch sees a non-SM90 GPU")
PY
'
...
run_stage "pipeline" bash -lc '
  ...
  tuned_config="${AUDIT_DIR}/runtime_tuned_config.yaml"
  python - <<PY
...
tuned = Path("${AUDIT_DIR}/runtime_tuned_config.yaml")
