...
OUT_ROOT="${OUT_ROOT:-/project_runs/$(date +%Y%m%d_%H%M%S)_NN_SOLID_1_2}"
MAX_ATTEMPTS="${MAX_ATTEMPTS:-6}"

export PYTHONPATH="/project_root/analysis_pipeline/src:${PYTHONPATH:-}"
...
while [[ "$attempt" -le "$MAX_ATTEMPTS" ]]; do
  ...
  python scripts/nn_solid_1_2_runner.py \
    ...
    --features_root_healthy /project_cache/features/confirmatory_reference \
    --features_root_mechanism /project_cache/features/mechanism_reference \
    ...
    --clinical_severity_csv /project_data/clinical_bids/clinical_severity.csv \
    --wall_hours 10 \
    --lawc_n_perm 50000 \
